      ...
      <div class="nav">
        <el-card class="box-card">
          ...
          >
            ...
            <el-menu-item v-for="item in navCate" :key="item.id" :index="item.id">{{item.name}}</el-menu-item>
          </el-menu>
        </el-card>
        <el-card class="box-card" v-if="cardShow == true">
          <h3 style="display: flex; justify-content: space-between">
            ...
            >
              ...
              cursor: pointer" @click="getMoreArticle(1)"> 更多 </span>
          ...
          <el-row :gutter="20" class="illRow">
            <el-col :span="6" v-for="item in latestArticle" :key="item.id" class="illpic">
              <div class="illCard">
        ...
        <el-card class="adviseArticle">
          <h3 style="display: flex; justify-content: space-between">
            ...
            <div
              ...
              "
            >
              <span v-if="moreButtonShow" style="color: #9e9e9e;
              cursor: pointer" @click="moreArticle(onId)"> 更多 </span>
              <i v-if="moreButtonShow" class="el-icon-caret-right"></i>
            </div>
          </h3>
          <el-divider class="ccy-drvider"></el-divider>
          <el-row :gutter="20" class="articleRow">
            <el-col :span="4.8" v-for="item in adviseArticle" :key="item.id">
              <div class="adviseArticleCard">
                <el-card :body-style="{ padding: '0px' }" class="adviseCard">
                  <img :src="item.pic" class="advisedImage" />
                  <div style="padding: 14px">
                    <span>{{ item.title }}</span>
                    <div class="bottom clearfix">
                      <span class="title">惠农网·农技学堂</span>
...
export default {
  ...
  methods: {
    async getArticlesTypes() {
      const { data: res } = await this.reqM2Service("/education/technicalArticlesTypes",'','get')
      // console.log(res)
      if (res.code !== 20000) {
    ...
    async getLateArticles() {
      const { data: res } = await this.reqM2Service("/education/technicalArticles/findByTime/1/8",'','get')
      // console.log(res)
      if (res.code !== 20000) {
    ...
    async getAdviseArticle() {
      const { data: res } = await this.reqM2Service(`/education/technicalArticles/findByRecommend/${this.Pagenum}/8`,'','get')
      console.log(res)
      if (res.code !== 20000) {
    ...
    async getArticlesByTypes(typeId) {
      const { data: res } = await this.reqM2Service(`/education/technicalArticles/search/searchByTypeId/${typeId}/${this.Pagenum}/16`,'','get')
      console.log(res)
      if (res.code !== 20000) {
    ...
    async getMoreArticle(Id) {
      this.onId = Id
      if (Id == 1) {
        const { data: res } = await this.reqM2Service(`/education/technicalArticles/findByTime/${this.Pagenum}/8`,'','get')
        if (res.code !== 20000) {
          return this.$message.error('获取该类型文章数据失败！')
        }
        this.cardShow = false
